...
function serverCore(req, resp) {
  ...
  try {
    ...
    if (req.method == "GET" || req.method == "DELETE") {
      ...
      if (dataObj.req.url.toLowerCase().indexOf("/login") != -1) {
        opType = "login";
        dataObj.ident = req.url.substr(req.url.lastIndexOf("/") + 1);
       //console.log("!!!!! dataObj iden : " + dataObj.ident);
        serverCorePart2(dataObj);
      } else if (dataObj.req.url.toLowerCase().indexOf("/getuser") != -1) {
        opType = "user";
        dataObj.ident = req.url.substr(req.url.lastIndexOf("/") + 1);
        //console.log("GETTING USER!!!!! dataObj iden : " + dataObj.ident);
        serverCorePart2(dataObj);
      } else if (dataObj.req.url.toLowerCase().indexOf("/getappointment") != -1) {
        opType = "appointment";
        dataObj.ident = req.url.substr(req.url.lastIndexOf("/") + 1);
        console.log("GETTING Appointment!!!!! dataObj iden : " + dataObj.ident);
        serverCorePart2(dataObj);
      }
      else {
        serverCorePart2(dataObj);
    ...
    // If it's a POST or PUT request then we have to get the body content.
      ...
      req.on("end", function() {
        ...
        // console.log("!!!!! AFTER PARSE: dataObj data : " + dataObj.data);
        serverCorePart2(dataObj);
      });
...
function serverCorePart2(dataObj) {
  ...
  try {
    ...
    if (dataObj.req.url.toLowerCase().indexOf("/appointment") != -1) {
      ...
      opType = "getappointment";
    } else if (dataObj.req.url.toLowerCase().indexOf("/user") != -1) {
      opType = "user";
    }else if (dataObj.req.url.toLowerCase().indexOf("/getuser") != -1) {
      opType = "getUser";
    } else if (dataObj.req.url.toLowerCase().indexOf("/login") != -1) {
      opType = "login";
    } else if (dataObj.req.url.toLowerCase().indexOf("/index") != -1) {
      opType = "index";
    } else {
    ...
    if (opType == "login"){
      DAO.LOGIN("user", dataObj);
    } else if (opType == "getUser"){
      DAO.GET("user", dataObj);
    } else if (opType == "getappointment"){
      DAO.GET("appointment", dataObj);
...
var server = http.createServer(serverCore).listen(port, ipaddr);
